package net.juweitang.tms.common.web.velocity.resolver;
...
public class FixedVelocityLayoutView extends VelocityToolboxView {
	...
	private void renderScreenContent(Context velocityContext) throws Exception {
		if (logger.isDebugEnabled()) {
			logger
					.debug("Rendering screen content template [" + getUrl()
							+ "]");
		}
		...
		if (TimeProfiler.isProfileEnable()) {
			TimeProfiler.beginTask("render screen:"
					+ screenContentTemplate.getName());
		}
		try {
