package net.juweitang.tms.common.rpc.hessian;
...
/**
 * {@link org.aopalliance.intercept.MethodInterceptor} for accessing a Hessian
 * service. Supports authentication via username and password. The service URL
 * must be an HTTP URL exposing a Hessian service.
 * 
 * <p>
 ...
 * to have been exported using Spring's {@link HessianServiceExporter}, as there
 * is no special handling involved. As a consequence, you can also access
 * services that have been exported using Caucho's
 * {@link com.caucho.hessian.server.HessianServlet}.
 * 
 ...
 * 
 * add by sam.zhang.��spring�汾������ setConnectTimeout
 *      ����
 */
public class HessianClientInterceptor extends UrlBasedRemoteAccessor implements
		MethodInterceptor {
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		try {
			return invocation.getMethod().invoke(this.hessianProxy,
			...
			if (targetEx instanceof HessianConnectionException) {
				...
				throw convertHessianAccessException(cause != null ? cause
						: targetEx);
